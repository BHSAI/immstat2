...
permutationTEST <- function(X, Y, r){
  ...
  write(data.table::setDT(df)[, mean(ROC), by=Rep]$V1, "/home/plu/rf_up_statusDay42_kenya_complete_AL_cv_20200806_aucroc", ncolumns = 1)
  write.csv(ls$finalModel$confusion, "/home/plu/rf_up_statusDay42_kenya_complete_AL_cv_20200806_finalModel.csv", row.names = TRUE, col.names = TRUE)
  write.csv(varImp[order(varImp$Overall,decreasing = TRUE),], "/home/plu/rf_up_statusDay42_kenya_complete_AL_cv_20200806_varImp.csv", row.names = TRUE, col.names = TRUE)
  prediction_data = ls$pred[ls$pred$mtry == ls$finalModel$mtry, ]
  write.csv(confusionMatrix(prediction_data$pred, prediction_data$obs)$table, "/home/plu/rf_up_statusDay42_kenya_complete_AL_cv_20200806_accuracyCV.csv",row.names = TRUE, col.names = TRUE)
  write(confusionMatrix(prediction_data$pred, prediction_data$obs)$overall[2], "/home/plu/rf_up_statusDay42_kenya_complete_AL_cv_20200806_Kappa")
...
X <- myData[which(pheno.df$Study.Arm == "AL" & !(is.na(pheno.df$ACPR42))),]
names(X) <- make.names(names(X))
Y <- pheno.df[which(pheno.df$Study.Arm == "AL" & !(is.na(pheno.df$ACPR42))),"ACPR42"]
Y <- as.factor(Y)
levels(Y) <- c("NP", "P")
output <- permutationTEST(X, Y, r)

lapply(output, print)
write.csv(output$permutes, "/home/plu/rf_up_statusDay42_kenya_complete_AL_cv_20200806.csv")
write(output$aucroc_permutation, "/home/plu/rf_up_statusDay42_kenya_complete_AL_cv_20200806_aucrocPerm", ncolumns = 1)

save.image("/home/plu/rf_up_statusDay42_kenya_complete_AL_cv_20200806.RData")
